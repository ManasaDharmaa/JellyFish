...
public class IDCard {
	...
	public IDCard(int age, String password, String name, long phoneNum, String email, boolean batch) {
		if (age > 18) {
		...
		if (letterCount == 5 && digitCount == 2 && upperCount == 1 && specialCount == 1) {
			...
			try {
				...
				System.out
						.println("your password not contains 7 letter or not 2 numbers or 1 charrecter or 1 uppercase");
		...
		if (name.length() < 8 && Character.isUpperCase(name.charAt(0))) {
			...
			try {
				...
				System.out.println(
						"Your name not contains 8 letters or it containt numbers or charecters or first letter not should be a upper case");
			}
		}
